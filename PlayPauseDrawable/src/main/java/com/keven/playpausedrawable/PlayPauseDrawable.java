...
public class PlayPauseDrawable extends Drawable {
    ...
    private boolean mAnimationMode = true;
    ...
    private int State = PAUSEING;

    public PlayPauseDrawable() {
        this(14, 0XFFE91E63, 0XFFffffff, 300, PAUSEING);
    }

    public PlayPauseDrawable(int strokeWidth, int playColor, int pauseColor, long animationDuration, int startState) {
        State = startState;
        mStrokeWidth = strokeWidth;
        mPlayColor = playColor;
        mPauseColor = pauseColor;
        mAnimationDuration = animationDuration;
        initPaints(startState);
    }

    private void initPaints(int startState) {
        ...
        if (startState == PLAYING) {
            mLinePaint.setColor( mPlayColor);
            mBackgroundPaint.setColor(mPauseColor);
        } else {
    ...
    private void setupPlayMode() {
        ...
        mPoints[2] = mBounds.right + shiftY;
        mPoints[3] = mBounds.centerY();

        mPoints[4] = mBounds.right + shiftY;
        mPoints[5] = mBounds.centerY();

        mPoints[6] = mBounds.left + shiftY;
        mPoints[7] = mBounds.bottom;
